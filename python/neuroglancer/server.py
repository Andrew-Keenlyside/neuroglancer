...
from .async_util import _PYODIDE
...
if not _PYODIDE:
    ...
    def stop() -> None:
        ...
        This allows any previously-referenced data arrays to be garbage collected if there are no
    ...
    # Pyodide mode: delegate to browser_server

    def set_static_content_source(*args, **kwargs):
        pass  # Not applicable in Pyodide mode
    ...
    def is_server_running() -> bool:
        return True  # Always "running" in Pyodide mode

    def stop() -> None:
        pass  # Not applicable in Pyodide mode

    def get_server_url() -> str:
        return ""  # Base URL is the page itself

    def start() -> None:
        pass  # No-op; browser_server is initialized elsewhere

    def register_viewer(viewer):
        from . import browser_server

        browser_server.get_browser_server().register_viewer(viewer)

    def defer_callback(callback, *args, **kwargs):
        """Register `callback` to run via JS setTimeout (Pyodide mode).
